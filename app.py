...
class QuantEngine:
    ...
    def get_earnings_date(ticker_symbol: str) -> str:
        ...
        # 3. 야후 API 데이터가 안 넘어오는 경우 수동 매핑 보완 예시 (예: ASTS 등 필요시 추가)
        manual_earnings = {
            "ASTS": "2026-08-10" # 예시 날짜 (실제 일정에 맞춰 수정 가능)
        }
        if ticker_symbol in manual_earnings:
            try:
                dt_kst = datetime.strptime(manual_earnings[ticker_symbol], "%Y-%m-%d").replace(tzinfo=timezone(timedelta(hours=9)))
                now_kst = datetime.now(timezone(timedelta(hours=9)))
                diff_days = (dt_kst.date() - now_kst.date()).days
                if -5 <= diff_days <= 2:
                    if diff_days >= 0:
